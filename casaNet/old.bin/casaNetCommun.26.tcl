...
proc bgerror {message} {
    global errorInfo
    errmsg "message = \"$message\", errorinfo = \"$errorInfo\""    
}
...
proc writeTunnel {ps data} {
    global TUNNEL
    # puts stderr "writeTunnel $ps data=\"$data\""
...
proc transmetsOut {sock} {
    ...
    set len [string length $bytes]
    if {$len != 0} {
        # puts stderr "transmetsOut : $len bytes on OUT($sock) = \"$OUT($sock)\""
