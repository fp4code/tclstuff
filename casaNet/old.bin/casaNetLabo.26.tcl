...
proc commande argum {
    ...
    switch $commande {
        ...
        "exit" {
            writeTunnel $TUNPS(stdout) "casaNetLabo hard exit !"
            exit 0
        }
    }
}
...
proc readMaison1 {maison} {
    ...
    writeTunnel $TUNPS(stderr) "essai erreur 1"
    writeTunnel $TUNPS(stderr) "essai erreur 2"
    writeTunnel $TUNPS(stderr) "essai erreur 3"
    ...
    after 2000 {writeTunnel $TUNPS(stdout) "encore 2 secondes"}
    after 4000 {writeTunnel $TUNPS(stdout) "encore 2 secondes de plus"}
    after 6000 [list demarre $display]
}

proc demarre {ecran} {
    global TUNPS
    set err [catch {exec /usr/openwin/bin/xterm -display $ecran &} m]
    writeTunnel $TUNPS(stderr) "no exec xterm : err=$err, m=\"$m\""
}


proc doIt {proxyHost proxyPort tunnelHost tunnelPort} {
