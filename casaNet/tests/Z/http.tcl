...
#
# RCS: @(#) $Id: http.tcl,v 1.1.1.1 2002/05/27 13:42:51 fab Exp $
...
proc getsNew {channel varName} {
    upvar $varName var
    set ret [gets $channel var]
    puts stderr "$channel => $var"
    return $ret
}

proc readNew {channel n} {
    set ret [read $channel $n]
    puts stderr "$channel -> $ret"
    return $ret
}
...
proc http::geturl { url args } {
	...
	set srvurl $url
        set commande [concat $defcmd $async {$phost $pport}]
        puts stderr [list $commande phost=$phost pport=$pport]
	set conStat [catch {eval $commande} s]
    } else {
	set conStat [catch {eval $defcmd $async {$host $port}} s]
    }
    puts stderr "conStat = $conStat"
    ...
    if {[catch {
	putsNew $s "$how $srvurl HTTP/1.0"
	putsNew $s "Accept: $http(-accept)"
	putsNew $s "Host: $host"
	putsNew $s "User-Agent: $http(-useragent)"
	...
	if {$isQuery || $isQueryChannel} {
	    putsNew $s "Content-Type: $state(-type)"
	    if {!$contDone} {
		putsNew $s "Content-Length: $state(querylength)"
	    }
	    putsNew $s ""
...
proc http::Write {token} {
	...
	if {[info exists state(-query)]} {
	    ...
	    set outStr [redNew $state(-querychannel) $state(-queryblocksize)]
	    putsNew -nonewline $s $outStr
...
#	Read the socket and handle callbacks.
 ...
 proc http::Event {token} {
	...
	if {[catch {
		...
		set block [redNew $s $state(-blocksize)]
...
http::config -proxyhost proxy -proxyport 8080
#set token [http::geturl http://yoko.lpn.prive]
#set data [::http::data $token]
# set token [http::geturl http://yoko.lpn.prive/fab-bin/test-cgi]
#set token [http::geturl http://tondu.lpn.prive]
#set data [::http::data $token]
set qq [::http::formatQuery cle1 valeur1 cle2 valeur2]
puts stderr "qq = $qq"
#set token [http::geturl http://tondu.lpn.prive -query $qq]

package require tls
http::register https 443 [list ::tls::socket -require 1 -cafile ./server.pem]
#http::register https 443 ::tls::socket
get token [http::geturl https://www.farnell.com/france]
puts stderr $token
set data [::http::data $token]
puts stderr $data
