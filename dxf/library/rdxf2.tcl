...
# 24 octobre 2001 (FP)
# AutoCAD 2002 v.u16.1.01, et réécrit pour lire AC1009
# Technique de lecture robuste, mais peu efficace parce qu'on est
# ammené à lire 3 fois le contenu des blocks par exemple
# 14 janvier 2002 (FP) je me repenche dessus (avec masque Picogiga)

package require fidev
package require fidev_zinzout

set FICHIER /home/fab/Y/picogiga.dxf
...
0 9 {String 
(with the introduction of extended symbol names in AutoCAD 2000, the 255 character limit has been lifted.
There is no explicit limit to the number of bytes per line, although most lines should fall within 2049 bytes)
}
...
proc dxf::cleancv {code value} {
    ...
    ($code == 1071)} {
        if {![regexp "^\[ \t\]*\[\\+|-\]?(\[1-9\]\[0-9\]*)|0\[ \t\]*\$" $value]} {
            error "Entier incorrect \"$value\" pour code $code"
        }
...
set INFO(dxf::readFile) {
    Lit le fichier ASCII $fichier
    et crée la liste de nom $cleanedLinesName
    avec le contenu de chaque ligne, les nombres étant interprétés.
    Retourne une liste des sections, chaque élément
    étant le nom, l'index de début du contenu et l'index suivant la fin
}

proc dxf::readFile {fichier cleanedLinesName} {
    upvar $cleanedLinesName cleanedLines
    ...
    for {set i 0} {$i < $NKV} {} {
        ...
        incr i
        lappend cleanedLines $code $value
        if {$code == 0} {
            if {$value == "SECTION"} {
                set code [expr {[lindex $lignes $i]}]
                incr i
                ...
                lappend cleanedLines $code $value
                if {$code != 2} {
                    error "Ligne $i, code attendu = 2, code vu = $code" 
                }
                lappend section $value $i
            } elseif {$value == "ENDSEC"} {
                lappend section [expr {$i - 2}]
                lappend sections $section
                set section [list]
            }
        }
    }
    if {[lindex [lindex $sections 0] 1] != 4} {
        puts stderr "Warning, choses avant la première section"
    }
    foreach section $sections {
        if {[llength $section] != 3} {
            puts stderr "Warning, section incorrecte : \"$section\""
        }
    }
    set endsec [lindex [lindex $sections 0] 2]
    foreach section [lrange $sections 1 end] {
        if {[lindex $section 1] != [expr {$endsec + 6}]} {
            puts stderr "Warning, choses avant section \"$section\""
        }
        set endsec [lindex $section 2]
    }
    if {[llength $section] != 3} {
        puts stderr "Warning, choses après la dernière section, ou fichier tronqué"
    } else {
        set eof [expr {[lindex $section 2] + 2}]
        if {[lindex $cleanedLines $eof] != 0 || [lindex $cleanedLines [expr {$eof+1}]] != "EOF"} {
            puts stderr "Warning, EOF manquant"
        } elseif {$eof + 2 != $NKV} {
            puts stderr "Warning, choses après EOF lignes [expr {$eof+3}] à $NKV"
        } 
    }
...
proc dxf::readHEADER {HEADERName lignes be} {
    upvar $HEADERName HEADER
    if {[info exists HEADER]} {unset HEADER}

    set begin [lindex $be 0]
    set end [lindex $be 1]
    set i $begin
    while {$i < $end} {
	set code [lindex $lignes $i]
        incr i
	...
	if {$code == 9} {
	    ...
	    set HEADER($name) [list]
            ...
            lappend HEADER($name) $code $value
        }
    }
}
	...
	set value [lindex $lignes $i]
        ...
        if {$code == 0} {
            if {$value == "TABLE"} {
                ...
                lappend table $value $i
            } elseif {$value == "ENDTAB"} {
                lappend table [expr {$i - 2}]
...
proc dxf::readTable {pName lignes be} {
    upvar $pName p
    if {[info exists p]} {
        unset p
    }
	...
	set value [lindex $lignes $i]
        ...
        if {$code == 0} {
            if {$value == $pName} {
                ...
                puts stderr "Warning, dans une table, code = 0, value attendue = $pName, vue = $value"
            }
        } else {
            lappend p($name) $code $value
        }
    }
}


set INFO(dxf::readENTITIES) {
0 type
  puis (ordre indifférent)
  5 handle
  8 layer
  autres sauf code 0
0 type
...
proc dxf::readENTITIES {objsName orderName lignes be} {
    upvar $objsName OBJS
    upvar $orderName ORDER ;# utile pour SEQEND

    if {[info exists OBJS]} {unset OBJS}
    if {[info exists ORDER]} {unset ORDER}
        ...
        if {$code == 0} {
            if {[info exists handle]} {
                if {![info exists layer]} {
                    puts stderr "Warning, pas de layer pour handle $handle"
                    set layer {}
                }
                if {[info exists OBJS($handle)]} {
                    return -code error "Le handle $handle existe déjà"
                }
                set OBJS($handle) [concat [list $type $layer] $reste]
                if {[info exists previous]} {
                    set ORDER($handle) $previous 
                    lappend ORDER($previous) $handle
                } else {
                    set ORDER($handle) {}
                }
                set previous $handle
                unset handle type layer
            }
            set reste [list]
            set type $value
        } elseif {$code == 5} {
            if {[info exists handle]} {
                puts stderr "Warning, deux codes 5 : $handle et $value"
            }
            set handle $value
        } elseif {$code == 8} {
            if {[info exists layer]} {
                puts stderr "Warning, deux codes 5 : $layer et $value"
            }
            set layer $value
        } else {
            lappend reste $code $value
        }
    }
    lappend ORDER($previous) {}
    return
}

set INFO(dxf::readBLOCKS) {
0 block
  puis (ordre indifférent)
  8 layer
  2 name
...
Retourne une liste d'éléments {nom debut fin handle layer liste_double_des_autres}

}


proc dxf::readBLOCKS {lignes be} {
	...
	set value [lindex $lignes $i]
        ...
        if {$code == 0} {
            if {$value == "BLOCK"} {
                if {$status == "endblk"} {
                    if {![info exists name]} {
                        puts stderr "Warning, block sans nom, ENDBLK en "
                        set name {}
                    }
                    set block [list $name $b [expr {$i-2}]]
                    unset name
                    lappend blocks $block
                } elseif {$status != "begin"} {
                    return -code error "BLOCK inattendu ligne $i"
                }
                set status block
                set b $i
            } elseif {$value == "ENDBLK"} {
                if {$status == "endblk"} {
                    return -code error "ENDBLK inattendu ligne $i"
                }
                set status endblk
            } else {
                if {$status != "entities"} {
...
set sections [dxf::readFile $FICHIER lignes]
catch {unset SECTIONS}
foreach section $sections {
    set sectionName [lindex $section 0]
    if {[info exists SECTION($sectionName)]} {
        error "Deux sections de nom $sectionName"
    }
    set SECTIONS($sectionName) [lrange $section 1 2]
}
unset sections
parray SECTIONS

dxf::readHEADER HEADER $lignes $SECTIONS(HEADER)
parray HEADER

set tables [dxf::readTABLES $lignes $SECTIONS(TABLES)]
foreach table $tables {
    set tableName [lindex $table 0]
    if {[info exists SECTION($tableName)]} {
        error "Deux tables de nom $tableName"
    }
    set TABLES($tableName) [lrange $table 1 2]
}
unset tables
parray TABLES

foreach table [array names TABLES] {
    dxf::readTable $table $lignes $TABLES($table)
    parray $table
}


dxf::readENTITIES OBJS ORDER $lignes $SECTIONS(ENTITIES)
...
proc dxf::readAndInterpretFile {fichier HEADERNAME CLASSESNAME TABLESNAME BLOCKSNAME ENTITIESNAME} {
    upvar $HEADERNAME HEADER
...
proc execute {} {
    ...
    set kvs [dxf::readAndInterpretFile $FICHIER HEADER CLASSES TABLES BLOCKS ENTITIES]
