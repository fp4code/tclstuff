...
proc+doc gds2::scanFile {file} {
    Étant donné un nom de fichier GDS-II $file, lit de fichier et renvoit une liste hiérarchique
} {
    %extern%
} {
...
proc+doc gds2::scanStream {stream} {
    Étant donnée une suite d'octets GDS-II $stream, construit la liste hiérarchique correspondante
} {
    gds2::scanFile
} {
    2004-03-02 (FP) proc+doc
...
proc+doc gds2::scanRecord {libName statusName stream i} {
    Lit à partir du mot $i de la chaine binaire GDS-II $stream un enregistrement GDS-II
    $statusName contient le nom d'une chaine ou bien d'un liste contenant l'état de l'interprétation du GDS-II
...
proc+doc gds2::formatLib {lib} {
    Retourne une chaine GDS-II à partir d'une liste hiérarchique $lib
} {
    gds2::writeToFile
} {  
    2004-03-02 (FP) proc+doc
    ...
    set gds2 ""
    append gds2 [gds2::aux::formatHEADER $version]
    append gds2 [gds2::aux::formatBGNLIB $modif_time $access_time]
    append gds2 [gds2::aux::formatLIBNAME $name]
    if {$reflibs != {}} {
        append gds2 [gds2::aux::formatREFLIBS $reflibs]
    }
    if {$fonts != {}} {
        append gds2 [gds2::aux::formatFONTS $fonts]
    }
    if {$attrtable != {}} {
        append gds2 [gds2::aux::formatATTRTABLE $attrtable]
    }
    if {$generations != {}} {
        append gds2 [gds2::aux::formatGENERATIONS $generations]
    }
    if {$formattype != {}} {
        append gds2 [gds2::aux::formatFormattype $formattype]
    }
    append gds2 [gds2::aux::formatUNITS $units]
    foreach structure $structures {
        append gds2 [gds2::aux::formatStructure $structure]
    }
    append gds2 [gds2::aux::formatENDLIB]
...
proc gds2::aux::formatRecord {type data} {
    ...
    switch $dataType {
        ...
        "01" {
            puts stderr "Binary Record $type $data pas bien traité"
	    set record {}
        }
        "02" {
...
proc+doc gds2::writeToFile {lib filename} {
    Crée le fichier GDS-II $filename à partir d'une liste hiérarchique $lib
} {
    %extern%
} {
    2004-03-02 (FP) proc+doc
    ...
    puts stderr "[clock format [clock seconds]] formatLib"
