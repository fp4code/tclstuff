...
proc low_level_ui {root args} {
	...
	button $base.button_DCL \
		-command {GPIB_Command DCL} \
		-text DCL

	button $base.button_SPE \
		-command {GPIB_Command SPE} \
		-text SPE

	button $base.button_SPD \
		-command {GPIB_Command SPD} \
		-text SPD

	button $base.button_UNL \
		-command {GPIB_Command UNL} \
		-text UNL

	button $base.button_UNT \
		-command {GPIB_Command UNT} \
		-text UNT

	button $base.button_GET \
		-command {GPIB_Command GET} \
		-text GET

	button $base.button_GTL \
		-command {GPIB_Command GTL} \
		-text GTL

	button $base.button_SDC \
		-command {GPIB_Command SDC} \
		-text SDC

	button $base.button_TCT \
		-command {GPIB_Command TCT} \
		-text TCT

	button $base.button_readByte \
		-command GPIB_read_Byte \
		-text {read Byte}

	button $base.button_rescan_ll \
		-command GPIB_rescanDevices_lowLevel \
		-text {rescan Devices}

	button $base.button#38 \
		-command {GPIB_Command2 MTA 0} \
		-text {MTA gpib0}

	button $base.button#39 \
		-command {GPIB_Command2 MLA 0} \
		-text {MLA gpib0}
