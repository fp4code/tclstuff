...
# 2008-12-23 (FP) Bug gpibNames: handle gpibBoard != 0
# 2012-09-12 4005 -> 4006
...
#############
    ...
    $B.conn_scrute configure -command {
            ...
            foreach n $numBranches {
                global gpibNames
                if {[info exists gpibNames($::GPIB_board,$n)]} {
                    lappend GPIB_Branches $gpibNames($::GPIB_board,$n)
                } else {
...
############
    ...
    button $B.pos.mm4006 \
            -text {MM 4006} -command {
        set TC(machine) mm4006
        if {[winfo exists .mm4006_ui]} {
            wm deiconify .mm4006_ui
            raise .mm4006_ui
        } else {
            package require tablexy 1.2
            package require mm4006 1.0
            mm4006::createIfNonExistent
            toplevel .mm4006_ui
            wm geometry .mm4006_ui +0+0
            aligned::tablexy_ui mm4006 mm4006::specialFrame
            aide::nondocumente .mm4006_ui
        }
    }
    aide::ui_minihelp $B.message $B.pos.mm4006 {démarre une séquence d'alignement}
...
#########
    ...
    grid configure $B.frame#1 -in $root    -row 0 -column 0 
        ...
        grid configure $B.pos.mm4006  $B.pos.calcule $B.pos.print  -sticky news
