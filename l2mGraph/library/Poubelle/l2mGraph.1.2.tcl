...
# FAIRE l2mGraphPriv_barycentric
	    set ncoords {}
	    foreach {x y} $coords {
...
puts  "FAIRE l2mGraphPriv_barycentric"
		foreach {x y} $coords {
		    if {$x < $xmin} {
			set xmin $x
		    }
...
::tcl::OptProc l2mGraph::plotSimple {
    {F {nom du l2mGraph}} 
    {xl {liste des x}}
    {yl {liste des y}}
    {-xscale -string x {nom de l'echelle x}}
    {-yscale -string y {nom de l'echelle y}}
    {-tags -list {} {tags}}
    {-style -choice {lines points croix}}
    {-typeOfPoints -string cross}
} {
    upvar #0 graphPriv$F G
	...
	global l2mGraph::bitmaps
	if {![info exists l2mGraph::bitmaps($typeOfPoints)]} {
	    set l2mGraph::bitmaps(points) [l2mGraph::creeBitmap $typeOfPoints]
	    foreach xp  $xlp yp $ylp {
		$F.c create image $xp $yp -image $l2mGraph::bitmaps(points) -anchor center -tags $tags
	    }
	}  
    } elseif {$style == "croix"} {
	global l2mGraph::bitmaps
	if {![info exists l2mGraph::bitmaps(croix)]} {
	    set l2mGraph::bitmaps(croix)\
		    [image create bitmap -data {
		#define croix_width 7
		#define croix_height 7
		#define croix_x_hot 3
		#define croix_y_hot 3
		static unsigned char croix_bits[] = {
		    0x08, 0x08, 0x08, 0x77, 0x08, 0x08, 0x08};
		}
	     ]
	    puts croix
            foreach xp  $xlp yp $ylp {
puts [list $F.c create image $xp $yp -image $l2mGraph::bitmaps(croix) -anchor center -tags $tags]
		$F.c create image $xp $yp -image $l2mGraph::bitmaps(croix) -anchor center -tags $tags
	    }
	}
    }    
}    
...
proc l2mGraph::plotWithErrors {F xlist ylist xscale yscale tags} {
    ...
    plotSimple $F $xl $yl -xscale $xscale -yscale  $yscale -tags $tags
}
