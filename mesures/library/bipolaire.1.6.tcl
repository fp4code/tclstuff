package provide mes_bipolaire 1.6
...
proc mes::controlLimit {thingName unit symDes thingPerUnitOfSurface} {
    global gloglo
    ...
    if {[info exists gloglo($thingName,$family)]} {
        set thing [readUnit $gloglo($thingName,$family) $unit]
...
set MES_BIPOLAIRE_SUBFACTEUR 0.05
set MES_BIPOLAIRE_FACTEUR 0.5

set rien {
    set MES_BIPOLAIRE_SUBFACTEUR 1.0
    set MES_BIPOLAIRE_FACTEUR 0.8
...
proc ::mes::bipolaire::diodeBx_DirFR {EouC nom} {
    ...
    mes::controlLimit IbDirMax A $symDes $JbDirMax

    set VbeDirMax [readUnit $gloglo(VbeDirMax) V]
    set VbcDirMax [readUnit $gloglo(VbcDirMax) V]

    smuE V(I)
    smuB I(V)
    smuC V(I)

    set idirmin [expr {-[readUnit $gloglo(ILogMin) A]}]
    set idirmax [expr {-$IbDirMax}]

    # aller-retour
    set ilist [smuE splitIRange $idirmin $idirmax $MES_BIPOLAIRE_FACTEUR $MES_BIPOLAIRE_SUBFACTEUR]
    set ilist [concat $ilist [smuE splitIRange $idirmax $idirmin $MES_BIPOLAIRE_FACTEUR $MES_BIPOLAIRE_SUBFACTEUR]]
...
proc ::mes::bipolaire::IbIxFR {EouC Vxb nom} {
    return [::mes::bipolaire::IbIxFRv2 1 1 $EouC $Vxb $nom]
}

set HELP(::mes::bipolaire::IbIxFRv2) {

}
proc ::mes::bipolaire::IbIxFRv2 {rangeFact_B rangeFact_CouE EouC Vxb nom} {
    global JceMaxRaisonnable
    global gloglo
    global MES_BIPOLAIRE_FACTEUR MES_BIPOLAIRE_SUBFACTEUR
    ...
    set JceMax [readUnit $gloglo(JceMax) "A*um-2"]
    ...
    mes::controlLimit IbDirMax A $symDes $JbDirMax
    mes::controlLimit IceMax A $symDes $JceMax

    set PMax [readUnit $gloglo(PMax) W*um-2]

    mes::controlLimit pmax W $symDes $PMax
    ...
    smu$EouC setCompliance [readUnit $gloglo(Vb${eouc}DirMax) V]
    smuB     setCompliance [expr {1.5*$IbDirMax}]
    smu$CouE setCompliance $IceMax

    set idirmin [expr {-[readUnit $gloglo(ILogMin) A]}]
    set idirmax [expr {-$IceMax}]

    # aller-retour
    set ilist [smu$EouC splitIRange $idirmin $idirmax $MES_BIPOLAIRE_FACTEUR $MES_BIPOLAIRE_SUBFACTEUR]
    set ilist [concat $ilist [smu$EouC splitIRange $idirmax $idirmin $MES_BIPOLAIRE_FACTEUR $MES_BIPOLAIRE_SUBFACTEUR]]

    set VbxDirMax [set Vb${eouc}DirMax]
    # gamme fixe
    smu$EouC setCompliance $VbxDirMax -range best

    ::mes::bipolaire::iniFR retour "@@IbI${coue} (V${coue} = $Vxb) $nom"
...
proc ::mes::bipolaire::IbIxFRLowCur {rangeFact_B rangeFact_CouE EouC Vxb nom} {
    ...
    set vDirMin [expr {-[readUnit $gloglo(Vb${eouc}LowCurMin) V]}]
    set vDirMax [expr {-[readUnit $gloglo(Vb${eouc}LowCurMax) V]}]
    set dVb [expr {-[readUnit $gloglo(dVb${eouc}) V]}]
    set delay 0

    smu$EouC linStairStep       $vDirMin $vDirMax   $dVb $delay
    smu$EouC linStairStepAppend $vDirMax $vDirMin [negVal $dVb] $delay
    set npts [smu$EouC getSweepSize]
    smuB     fixedLevelSweep 0.0 0 $npts -range 1
    smu$CouE fixedLevelSweep $Vxb $delay $npts -range [::smu::bestVRange smu$CouE $Vxb]
    ::mes::bipolaire::3smu.mesureFR retour
...
proc ::mes::bipolaire::diodeBx_InvFR {EouC nom} {
    global gloglo
    ...
    mes::controlLimit IbInvMax A $symDes [readUnit $gloglo(J${eouc}InvMax) A*um-2]
    ...
    set vDirRacc [expr {-[readUnit $gloglo(Vb${eouc}DirRaccord) V]}]
    set vInvMax  [expr {[readUnit $gloglo(Vb${eouc}InvMax) V]}]
    set step [readUnit $gloglo(dVb${eouc}Inv) V]
...
#	after 500
#    }
    smu$EouC linStairStepAppend $vInvMax  $vDirRacc [negVal $step] 0 -range $vInvMaxRange
#    if {$EouC == "C"} {
#	puts -nonewline stderr {rustine à revoir pb. K0 NRFD : on roupille 500 ms...}
#	after 500
#    }
...
# sans range, c'est long !
    smu$CouE fixedLevelSweep 0.0 0.0 $npts -range $IbInvMaxRange

    return [::mes::bipolaire::3smuMesAndResult "@@diodeB$EouC Inv (I${coue}=0) $nom"]
}


proc ::mes::bipolaire::IcVceFR {IbFractionDeIceMax betaMax nom} {
    global JceMaxRaisonnable
    global gloglo
    ...
    mes::controlLimit IcMax2 A $symDes $JceMax

    if {$IcMax > $IcMax2} {
	set IcMax $IcMax2
    }

    mes::controlLimit pmax W $symDes [readUnit $gloglo(PMax) W*um-2]

    set VceMax [expr {$pmax/$IcMax}]
    set VceMax2 [readUnit $gloglo(VbcInvMax) V]
    if {$VceMax > $VceMax2} {
	set VceMax $VceMax2
    }
    ...
    smuC linStair       [negVal $Ib] 0.0 $gloglo(IcVceNptsIm) 0 -range $ibrange
    smuC linStairAppend 0.0 [negVal $Ib] $gloglo(IcVceNptsIm) 0 -range $ibrange
...
proc ::mes::bipolaire::diodeBx_Dir_PretriFR {EouC nom} {
    global JceMaxRaisonnable
    global gloglo
    global variable_SRQ
    ...
    smuB setCompliance [readUnit $gloglo(Vb${eouc}DirMax) V] -range best

    set idirmin [readUnit $gloglo(ILogMin) A]
...
##################################################################
#                    pretri (quick and dirty)                    #
# 1 smu en rampe et 2 smu en polarisation constante sans mesure  #
##################################################################

proc ::mes::bipolaire::diodeBx_Dir_Pretri {EouC nom} {
    global JceMaxRaisonnable
    global gloglo
    global variable_SRQ
    ...
    switch $gloglo(autreJonction) {
	"CC"     {set retour [list "@@diodeB$EouC Dir Pretri(Vb${coue}=0) $nom"]}
	"ouvert" {set retour [list "@@diodeB$EouC Dir Pretri(Ib${coue}=0) $nom"]}
	default {error "gloglo(autreJonction) should be \"CC\" or \"ouvert\""}
    }
    ...
    switch $EouC {
	"E" {
	    smuE setCompliance [readUnit $gloglo(VbeDirMax) V]
	}
	"C" {
	    smuC setCompliance [readUnit $gloglo(VbcDirMax) V]
	}
    }
    ...
    switch $gloglo(autreJonction) {
	"CC"     {smu$CouE setCompliance [expr {1.5*$IbDirMax}]}
	"ouvert" {smu$CouE setCompliance 2}
	default {error "gloglo(autreJonction) should be \"CC\" or \"ouvert\""}
    }
    ...
    smu$EouC trigIn continuous

    smu$EouC logStair       $idirmin $idirmax 5 0 ;# -range 1mA
    smu$EouC logStairAppend $idirmax $idirmin 5 0 ;# -range 1mA

    smuB     sourceContinue 0
    smu$CouE sourceContinue 0
    smu$EouC trigOn get

    smu$EouC operate

    smu$EouC SRQon Warning Error ReadyForTrigger
    smu$EouC waitRft
    smu$EouC SRQon Warning Error SweepDone
    if {$variable_SRQ != {}} {
        puts stderr "ERREUR : SRQ :"
        iv:testpoll
        error "ERREUR : SRQ !!!"
    ...
    set sx [smu$EouC litSweep]

    if {[lindex $sx 0] != [list @ I V instant statut]} {
         error "conflit mes.bipolaire.diodeBx_Dir_Pretri/::smu::litSweep sur smu$EouC"
    }
