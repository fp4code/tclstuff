# package provide mes_hemt 1.9

# 2007-03-07 issu de mes_bipolaire 1.8
# 2007-03-14 déverminage
# 2009-02-13 (FP) ajout IdVgsFR
# 2009-10-14 (FP) ajout de IdVgAR2
# 2012-11-20 (FP) ajout VdVgsFR VdVgsAR

# Le suffixe FR veut dire "Fixed Range"


set info {
   ::mes::hemt::IdVdsFR $Vgs
mesure en imposant Vgs et en balayant Vds entre 0 et VdsMax, fixed range

   ::mes::hemt::IdVdsAR $Vgs
mesure en imposant Vgs et en balayant Vds entre 0 et VdsMax, auto range

   ::mes::hemt::IdVgsFR $Vds 
mesure en imposant Vds et en balayant Vgs entre VgsA et VgsB, fixed range

   ::mes::hemt::IdVgsAR $Vds 
mesure en imposant Vds et en balayant Vgs entre VgsA et VgsB, auto range

   ::mes::hemt::IdVgAR2 $Vs $Vd
mesure en imposant Vs et Vd et en balayant Vg entre VgsA et VgsB

   ::mes::hemt::IVuniv $VsA $VsB $VgA $VgB $VdA $VdB $N
mesure universelle interpolant entre A et B

   ::mes::hemt::VdVgsFR $Vds 
mesure en imposant Vds et en balayant Vgs entre VgsA et VgsB, fixed range

   ::mes::hemt::VdVgsAR $Vds 
mesure en imposant Vds et en balayant Vgs entre VgsA et VgsB, auto range
...
puts stderr OK
    ...
    set aTester [list smuG smuS smuD]
    while {$aTester != {}} {
...
proc ::mes::hemt::IdVgAR2 {Vs Vd args} {
    global JdMaxRaisonnable
    ...
    ::mes::hemt::iniFR retour "@@IdVgs (Vs = $Vs, Vd = $Vd) $nom"
    ...
    set vgrange [smuG bestRangeFromList V [list $VgsA $VgsB]]
    set vsrange [smuS bestRangeFromList V $Vs]
    set vdrange [smuD bestRangeFromList V $Vd]
    smuG linStair       $VgsA $VgsB $gloglo(IdVgsNptsV) 0 -range $vgrange
    smuG linStairAppend $VgsB $VgsA $gloglo(IdVgsNptsV) 0 -range $vgrange
    set npts [smuG getSweepSize]
    smuS fixedLevelSweep $Vs 0 $npts -range $vsrange
    smuD fixedLevelSweep $Vd 0 $npts -range $vdrange
...
proc ::mes::hemt::IVuniv {VsA VsB VgA VgB VdA VdB N args} {
    global JdMaxRaisonnable
    ...
    puts "$VsA $VsB $VgA $VgB $VdA $VdB $N"
    ::mes::hemt::iniFR retour "@@IVuniv Vs=$VsA..$VsB Vg=$VgA..$VgB Vd=$VdA..$VdB $nom"
    ...
    set vsrange [smuS bestRangeFromList V [list $VsA $VsB]]
    set vgrange [smuG bestRangeFromList V [list $VgA $VgB]]
    set vdrange [smuD bestRangeFromList V [list $VdA $VdB]]

    set NN [expr {2*$N + 2}]

    if {$VsA != $VsB} {
	smuS linStair       $VsA $VsB $N 0 -range $vsrange
	smuS linStairAppend $VsB $VsA $N 0 -range $vsrange
    } else {
	smuS fixedLevelSweep $VsA 0 $NN -range $vsrange
    }
    if {$VgA != $VgB} {
	smuG linStair       $VgA $VgB $N 0 -range $vgrange
	smuG linStairAppend $VgB $VgA $N 0 -range $vgrange
    } else {
	smuG fixedLevelSweep $VgA 0 $NN -range $vgrange
    }
    if {$VdA != $VdB} {
	smuD linStair       $VdA $VdB $N 0 -range $vdrange
	smuD linStairAppend $VdB $VdA $N 0 -range $vdrange
    } else {
	smuD fixedLevelSweep $VdA 0 $NN -range $vdrange
    }
...
proc ::mes::hemt::VdVgsAR {args} {
	...
	set nom [lindex $args 0]
	puts stderr [list args = $args nom = $nom]
    } elseif {[llength $args] == 2 && [lindex $args 0] == "-params"} {
	upvar \#0 [lindex $args 1] gloglo
	set nom "[lindex $args 2] -params [lindex $args 1]"
    } else {
	return -code error "ERREUR, mauvais arguments supplémentaires \"$args\""
    ...
    ::mes::hemt::iniFR retour "@@VdVgs (Id = 0) $nom"

    smuD V(I)
    smuG I(V)
    smuS I(V)
    
    smuD setCompliance $VdsMax -range 0
    smuG setCompliance $IgMax -range 0
    smuS setCompliance $IdMax -range 0

    set vgrange [smuG bestRangeFromList V [list $VgsA $VgsB]]
    set idrange [smuD bestRangeFromList V 1e-9]
    smuG linStair       $VgsA $VgsB $gloglo(IdVgsNptsV) 0 -range $vgrange
    smuG linStairAppend $VgsB $VgsA $gloglo(IdVgsNptsV) 0 -range $vgrange
    set npts [smuG getSweepSize]
    smuS fixedLevelSweep 0.0 0 $npts -range 1
    smuD fixedLevelSweep 0.0 0 $npts -range $idrange
