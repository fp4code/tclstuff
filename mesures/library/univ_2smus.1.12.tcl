...
# 2016-10-27 (FP) introduction de gloglo(range). Par défaut "best" (0(auto) fatigue les relais des SMUs)
# 2016-10-28 (FP) crée IVuniv_fixed_range
...
proc ::mes::univ_2smus::IVuniv {VXA VXB VYA VYB N args} {
    ...
    if {[info exists gloglo(rangeX)]} {
	set range $gloglo(rangeX)
    } else {
	set rangeX best
    }
    if {[info exists gloglo(rangeY)]} {
	set range $gloglo(rangeY)
    } else {
	set rangeY best
    }

    puts "smuX setCompliance $IXMax -range $rangeX"
    puts "smuY setCompliance $IYMax -range $rangeY"
    
    smuX setCompliance $IXMax -range $rangeX
    smuY setCompliance $IYMax -range $rangeY
...
proc ::mes::univ_2smus::IVuniv_fixed_range {VXA VXB VYA VYB IXMax IYMax delayX delayY writeX writeY N args} {
    global JdMaxRaisonnable
    ...
    puts "$VXA $VXB $VYA $VYB $IXMax $IYMax $N"
    ::mes::univ_2smus::iniFR retour "@@IVuniv_fixed_range VX=$VXA..$VXB VY=$VYA..$VYB $nom"
    ...
    puts "smuX setCompliance $IXMax -range best"
    puts "smuY setCompliance $IYMax -range best"
    
    smuX setCompliance $IXMax -range best
    smuY setCompliance $IYMax -range best
    ...
    if {$VXA != $VXB} {
	smuX linStair       $VXA $VXB $N $delayX -range $vXrange
	smuX linStairAppend $VXB $VXA $N $delayX -range $vXrange
    } else {
	smuX fixedLevelSweep $VXA $delayX $NN -range $vXrange
    }
    if {$VYA != $VYB} {
	smuY linStair       $VYA $VYB $N $delayY -range $vYrange
	smuY linStairAppend $VYB $VYA $N $delayY -range $vYrange
    } else {
	smuY fixedLevelSweep $VYA $delayY $NN -range $vYrange
    }
...
package provide mes_univ_2smus 1.12
