...
proc readSFile {fichname} {
    ...
    set entete [lrange $lignes 0 11]
    set lcolnames [lindex $lignes 12]
    set ldata [lrange $lignes 13 end]
    
    set B1 "\[ \t\]"
    set BN "\[ \t\]*"
    set BS "\[ \t\]+"
        
    interprete "^!PLAQUE:${B1}(.*)"        [lindex $entete 0] params(PLAQUE) 
    interprete "^!TYPE:${B1}(.*)"          [lindex $entete 1] params(TYPE) 
    interprete "^!VARIETE:${B1}(.*)"       [lindex $entete 2] params(VARIETE) 
    interprete "^!Gravure\\(um\\):${B1}(.*)" [lindex $entete 3] params(Gravure)     

    interprete "^!POSIT:${B1}(.*)\$"       [lindex $entete 4] params(POSIT)
    interprete "^!DATE:${B1}(.*)${B1}HEURE:${B1}(.*)\$" [lindex $entete 5] params(DATE) params(HEURE)
          
    interprete "^!POLAR:${B1}(.*)"         [lindex $entete 6] params(POLAR)     
    interprete "^!fichier:${B1}(.*)"       [lindex $entete 7] params(fichier)     
    interprete "^!MES:${B1}(.*)"           [lindex $entete 8] params(MES1)     
    interprete "^!${B1}(.*)"               [lindex $entete 9] params(MES2)     
    
    interprete "^!\\\[S\\\] brut\$" [lindex $entete 10]

    interprete "^!FREQ S11 S12 ..\$" [lindex $entete 11]
