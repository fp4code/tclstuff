...
proc clean {fichier repertoire} {
    ...
    set ddir [concat $repertoire [lrange $fsplit 0 end-1]]
...
if {$argc < 5} {
    puts stderr "Syntaxe : $argv0 1 0.00041 2.2e-6 defalques.xxx fichiers_spt..."
    return
}
...
set fichiers [lrange $argv 4 end]
#foreach a $argv {
...
set PMOD(Z0) 50
set PMOD(Zrel) [lindex $argv 0]
set PMOD(nL) [lindex $argv 1]
set PMOD(Rskin) [lindex $argv 2]
set repertoire [lindex $argv 3]

if {$repertoire == "defalques"} {
    if {$PMOD(Zrel) !=1 || $PMOD(nL) != 0.00041 || $PMOD(Rskin) != 2.2e-6} {
        puts stderr "Le répertoire \"defalques\" est réservé aux paramètres 1 0.00041 2.2e-6"
        return
    }
} elseif {![string match defalques.* $repertoire]} {
    puts stderr "Les répertoire \"$repertoire\" n'est pas de la forme \"defalques.xxx\""
    return
}

foreach fichier $fichiers {
    if {[catch {clean $fichier $repertoire} message]} {
