...
package require hyperpianoHBT
package provide hyperHBT 1.0

namespace eval hyperHBT {}
...
set HELP(sparams::HexchangeBE) {
    12 avril 2001 (FP)
    retourne la matrice h en base commune
    à partir de la matrice en émetteur commun
}

proc sparams::HexchangeBE {h} {
    set hn [blas::vector create doublecomplex -length [blas::vector length $h]]

    set n [expr {[blas::vector length $h]/4}]

    set h11 [blas::subvector create 1 4 $n from h]
    set h12 [blas::subvector create 2 4 $n from h]
    set h21 [blas::subvector create 3 4 $n from h]
    set h22 [blas::subvector create 4 4 $n from h]

    set h11n [blas::subvector create 1 4 $n from hn]
    set h12n [blas::subvector create 2 4 $n from hn]
    set h21n [blas::subvector create 3 4 $n from hn]
    set h22n [blas::subvector create 4 4 $n from hn]

    blas::zcopy $h11 h11n
    blas::zcopy $h21 h12n ;# sic
    blas::zcopy $h12 h21n ;# sic
    blas::zcopy $h22 h22n

    blas::mathsvop h12n *rscal -1.0
    blas::mathsvop h21n *rscal -1.0
    ...
    blas::mathsvop det * $h22n
    blas::mathsvop tmp * $h21n
    blas::mathsvop det - $tmp
    blas::mathsvop det inverse
    
    blas::mathsvop h11n * $det
    blas::mathsvop h12n * $det
    blas::mathsvop h21n * $det
    blas::mathsvop h22n * $det
...
set HELP(sparams::HexchangeBE_viaS) {
    11 avril 2001 (FP)
    retourne la matrice h en base commune
    à partir de la matrice en émetteur commun
    Idem sparams::HexchangeBE
}

proc sparams::HexchangeBE_viaS {h} {
    set stmp $h
    sparams::transform SfromH stmp
    set stmp [sparams::SexchangeBE $stmp]
    sparams::transform HfromS stmp
    return $stmp
}

set HELP(hyperHBT::depouille) {
    12 avril 2001 (FP)
    retourne, notant hBk la matrice en base commune du coeur
    $hBk12 $hBk22 1/$hBk11 $hBk21/$hBk11
}

proc hyperHBT::depouille {s omega &Q} {
    upvar ${&Q} Q
    global 2M_PI
...
# Il est plus court de créer ces vecteurs que de les transmettre par adresse
    ...
    set f1  [blas::vector create doublecomplex -length $nelems]
    set fn  [blas::vector create doublecomplex -length $nelems]
    set f2  [blas::vector create doublecomplex -length $nelems]
    set f3  [blas::vector create doublecomplex -length $nelems]
    set stmp  [blas::vector create doublecomplex -length [expr {4*$nelems}]]
    set tmp  [blas::vector create double -length $nelems]
    ...
    blas::mathsvop f1 *rscal 0.0
    blas::mathsvop f1 +rscal [expr {-double($Q(Rp))*$unsurZ0}]
    blas::zcopy $f1 f2
    blas::mathsvop f3 *rscal 0.0
    ...
    blas::dcopy $omega tmp
    blas::mathsvop tmp *scal  [expr {-$Q(Z0)*$Q(Cp)}]    
    blas::mathsvop f1 +i $tmp
    blas::zcopy $f1 f2
    ...
    blas::dcopy $omega tmp
    blas::mathsvop tmp sqrt
    blas::mathsvop tmp *scal [expr {-$Q(Rskin)*$unsurZ0}]
    blas::mathsvop f1 +r $tmp
    blas::zcopy $f1 f2
    ...
    sparams::SofLine stmp $Q(Zrel) [expr {-$Q(nL)/299792458.}] $omega
    ...
    sparams::z+R//C f1 tmp $omega $Q(RbS) $Q(CbS)
    sparams::z+R+L f1 tmp $omega $Q(Rb) $Q(Lb)
    blas::mathsvop f1 *rscal [expr {-$unsurZ0}]
    
    sparams::z+R//C f2 tmp $omega $Q(RcS) $Q(CcS)
    sparams::z+R+L f2 tmp $omega $Q(Rc) $Q(Lc)
    blas::mathsvop f2 *rscal [expr {-$unsurZ0}]

    sparams::z+R//C f3 tmp $omega $Q(ReS) $Q(CeS)
    sparams::z+R+L f3 tmp $omega $Q(Re) $Q(Le)
    blas::mathsvop f3 *rscal [expr {-$unsurZ0}]
    ...
    blas::dcopy $omega tmp
    blas::mathsvop tmp *scal  [expr {-$Q(Z0)*$Q(ce_ext)}]    
    blas::mathsvop f2 +i $tmp

    blas::dcopy $omega tmp
    blas::mathsvop tmp *scal  [expr {-$Q(Z0)*$Q(cc_ext)}]    
    blas::mathsvop f3 +i $tmp
    ...
    blas::mathsvop f1 +rscal [expr {double(-$Q(rb))*$unsurZ0}]

    sparams::SwithZ s $f1 $f2 $f3

    ####################################

    sparams::transform HfromS s

    set hBk [sparams::HexchangeBE $s]

    blas::zcopy [blas::subvector create 1 4 $nelems from hBk] f2  ;# hBk11
    blas::zcopy [blas::subvector create 2 4 $nelems from hBk] fn  ;# hBk12
    blas::zcopy [blas::subvector create 3 4 $nelems from hBk] f3  ;# hBk21
    blas::zcopy [blas::subvector create 4 4 $nelems from hBk] f1  ;# hBk22

    blas::mathsvop f2 inverse
    blas::mathsvop f3 * $f2

    return [list $fn $f1 $f2 $f3]
}
...
#puts stderr "plotout $PT"
    ...
    foreach {Exp_kA Exp_kB Exp_kC Exp_kD} [hyperHBT::depouille $Exp_s $Exp_omega PMOD] break
    foreach {Exp0_kA Exp0_kB Exp0_kC Exp0_kD} [hyperHBT::depouille $Exp_s $Exp_omega PMOD0] break

    foreach {Mod_kA Mod_kB Mod_kC Mod_kD} [hyperHBT::depouille $Mod_s $Mod_omega PMOD] break
    foreach {Mod0_kA Mod0_kB Mod0_kC Mod0_kD} [hyperHBT::depouille $Mod0_s $Mod0_omega PMOD] break
...
#        toplor UBis$diese UBis
        ...
        foreach x {kA kB kC kD} {
            toplo $x$diese $x
        }

        toplok k$diese k $PMOD(Z0) PMOD
        # toplor UD$diese UD
