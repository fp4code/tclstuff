package provide tkstcb_gnuplot 2.4
...
# pour Win-XP, qui a l'air de coincer !
proc putput {channel data} {
    set max 0
    set n 0
    ...
    puts stderr "putput : $n lignes, max = $max"
}
proc putputnonewline {channel data} {
    set max 0
    set n 0
    set ldata [split $data \n]
    foreach l [lrange $ldata 0 end-1] {
	set len [string length $l]
	if {$max<$len} {set max $len}
	incr n
	puts $channel $l
    }
    set l [lindex $ldata end]
    set len [string length $l]
    if {$max<$len} {set max $len}
    incr n
    puts -nonewline $channel $l
    puts stderr "putputnonewline : $n lignes, max = $max"
}
...
# puts stderr [list gnuplotDirectWithCleanCols $cleancolnames $tkTable $colnames]
    ...
    set datas ""
    foreach ili [tkSuperTable::toutesLignesValides $tkTable] {
	set noOk 0
...
# puts [concat $computeGpDatas [list $tkTable $colnames]]
	...
	puts $privArray(channel) $commande
 	# putput $privArray(channel) $commande
	...
	if {[info exists privArray(heldDatas)] && $privArray(heldDatas) != ""} {
	    ...
	    # putputnonewline $privArray(channel) $privArray(heldDatas)
	}
	if {$privArray(debug)} {
	...
	for {set i $nNewCourbes} {$i > 0} {incr i -1} {
	    # pgnuplot/msys/XP n'aime pas les grosses chaines
	    # putput $privArray(channel) $datas
...
proc tkSuperTable::callbacks::externExec {gW programEntry colsEntry} {
    ...
    # putput $privArray(channel) $retour
...
proc tkSuperTable::callbacks::createGnuplot {} {
	...
	# il faudrait tester privArray(plotEnCours)
        ...
        # set privArray(channel) [open "|c:/prog/msys/prog/gnuplot-3.7.2/bin/pgnuplot.exe 2>@ stderr" w]
        set privArray(channel) [open "|/usr/bin/gnuplot 2>@ stderr" w]
