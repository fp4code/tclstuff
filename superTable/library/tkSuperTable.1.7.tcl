package provide tkSuperTable_alpha 1.7
...
proc tkSuperTable::nogood {win repertoire fichier spt} {
    puts $win
    set twin $win.nogood
    if {![winfo exists $twin]} {
	toplevel $twin
	text $twin.txt
	button $twin.save -text save -command "::widgets::saveIt $twin.txt $repertoire nogood.spt {{{table des mauvais} nogood*.spt}}"
	pack  $twin.save -fill x -expand y -side bottom
	::widgets::packWithScrollbar $twin txt
	$twin.txt insert end "@@table des mauvais\n"
	$twin.txt insert end "@repertoire fichier table\n"
    }
    $twin.txt insert end "[list $repertoire $fichier $spt]\n"
    $twin.txt see end
...
proc tkSuperTable::voitSpt {win args} {
	...
	} elseif {$gonogo == "-nogood"} {
	    tkSuperTable::nogood $win $repertoire $fichier $spt
	} else {
	    error "argument \"$gonogo\" ni \"-good\" ni \"-nogood\""
	}
	return
    }
