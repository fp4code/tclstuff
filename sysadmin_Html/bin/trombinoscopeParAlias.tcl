...
# 2008-02-18 changement de piece_et_phone
# 2008-04-21 ajout de alias_comment
# 2008-09-22 soda -> bob
...
foreach l [readLines alias_comment] {
    if {[regexp {^([^ 	]+)[ 	]*=[ 	]*([^ 	]*.+[^ 	]*)[ 	]*$} $l tout alias comment]} {
	set ALICOM($alias) $comment
    }
}
parray ALICOM
...
proc avertissement {body} { 
    ...
    xh::T $ts "Ce trombinoscope est OBSOLÈTE depuis de nombreuses années, après que le LPN ait décidé d'abandonner la norme historique imposant des logins de 8 caractères. Le "
    xh::ET $ts a [list href http://arclight.lpn.prive/html/soda/alias-courriel.txt] "fichier de référence"
    xh::T $ts " est OBSOLÈTE depuis la migration vers zimbra."
    xh::T $ts " (note : X = inconnu de la base LDAP, * : situation administrative inconnue)"
...
#    xh::E $head link {type text/css href /css/ibm_v13r1.css rel stylesheet}
    ...
    if {[info exists PIETE($login)]} {
	set infos $PIETE($login)
	set tr [xh::E $2cols tr]    
	set tkey [xh::E $tr td]
	set tvalue [xh::E $tr td]
	xh::T $tkey "phone"
	xh::T $tvalue [lindex $infos 1]
	set tr [xh::E $2cols tr]    
	set tkey [xh::E $tr td]
	set tvalue [xh::E $tr td]
	xh::T $tkey "pièce"
	xh::T $tvalue [lindex $infos 2]
	set tr [xh::E $2cols tr]    
	set tkey [xh::E $tr td]
	set tvalue [xh::E $tr td]
	xh::T $tkey "statut"
	xh::T $tvalue [lindex $infos 3]
	set tr [xh::E $2cols tr]    
	set tkey [xh::E $tr td]
	set tvalue [xh::E $tr td]
	xh::T $tkey "xls"
	xh::T $tvalue [lindex $infos 0]
    } else {
        puts stderr "Pas de données pour $regular"
    }
...
#    xh::E $td img {src "images/site_navig_haut.jpg" height 28 width 91 ALT "alias :"}
    ...
    if {[info exists ::ALICOM($alias)]} {
	set alicom " : $::ALICOM($alias)"
    } else {
	set alicom " : alias indéterminé"
    }
    xh::ET [xh::E $tdtitre center] b {} "$alias$alicom"
...
#    set tr1 [xh::E $table tr {height 20%}]
	...
	if {![info exists LOGIN($regular)]} {
	    ...
	    if {![info exists PIETE($LOGIN($regular))]} {
		append name "*"
	    }
	}
        if {[file exists [file join $BASE $DATABASES trombines $regular.gif]]} {
