...
proc secureputs {sock data timeout} {
    ...
    while {$PUTITRESTE > 0 && $PUTITDONE == 0} {
        ...
        puts stderr "PUTITRESTE = $PUTITRESTE -> \"$reste\""
...
proc getIt {sock n} {
    ...
    puts "lu $nlu sur $n ([string range $s 0 2]...[string range $s end-2 end]), reste $GETITRESTE"
...
proc doOneTest {BIGNAME timeout} {
    ...
    if [catch {
        secureputs $sock "N=[string length $BIG]\n" $timeout
    ...
    set clientprog {
        ...
        proc read_sock {sock} {
            ...
            if [eof $sock] {
                ...
                puts stderr "N:=$n"
                puts stderr [list read $sock $n]
                set l [read $sock $n]
                puts "client a lu [string length $l] octets sur $n"
