...
proc code_dur {&l} {
    upvar ${&l} l
    lappend l "si :\tOlivier.Oria,\tLorenzo.Bernardi,\tEric.Eusebe"
    lappend l "sos-si :\tOlivier.Oria,\tLorenzo.Bernardi,\tEric.Eusebe"
}

set AUTRES_ALIAS {contact helium jug projet-web projet-web-bd projet-web-cg projet-web-s rapac seminaires si sos-si}

set AUTRES_ALIAS {jug helium contact centrale-techno seminaires projet-web projet-web-bd projet-web-cg projet-web-s webmestre lepetitnouveau rapac cnanoidf si sos-si}
...
puts stderr "taper avant\n    ssh fecarabos 'ypcat passwd'  >| ~/Z/passwd\net\n    wget http://www.lpn.prive/web-si/utilisateurs/alias-courriel.txt -O ~/Z/alias-courriel.txt"
...
proc creeDatabaseLogin {lines} {
    ...
    foreach l $lines {
        set l [split $l :]
	puts stderr [list [lindex $l 0]  [lindex $l 2]  [lindex $l 3] [lindex $l 4]]
...
puts stderr "feuilles = $feuilles"
puts stderr "feuillesMultibranches = $feuillesMultibranches"
puts stderr "brindilles = $brindilles"
...
proc aliases_calculeInverses {aliasArrayName inverseArrayName regularName errorsName} {
	...
	puts stderr "left = $left, rightOrig = $aliasArray($left)"
        foreach rightOrig $aliasArray($left) {
            set right [string tolower $rightOrig]
            if {![info exists regular($right)]} {
		puts stderr "no regular($right)"
		puts stderr "aliasArray($left) = $aliasArray($left)"
...
proc aliases_longConnect_recurs {feuille noeud inverseArrayName longConnectName errorsName} {
	...
	puts stderr "exists $inverseArrayName\($noeud\)"
	return
    }
