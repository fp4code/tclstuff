...
namespace eval isometrie {
    ...
    set ::HELP(::isometrie::echToMec) {
        retourne la liste des 2 coordonnées échantillon à partir
        des coordonnées mécaniques $px et $py pour une isométrie donnée
    }
    proc mecToEch {name px py} {
        upvar #0 $name iso
        set px [expr {$px - $xTr}]
        set py [expr {$py - $yTr}]
    ...
    proc insertLast {name xymec xyech} {
        upvar #0 $name iso
    ...
    proc translateOnly {name xymec xyech} {
        upvar #0 $name iso
        if {![array exists iso] ||
