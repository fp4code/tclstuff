package provide tablexy 1.5
package require aligned 1.1

# 4 février 2002 (FP) 1.3   "incr" supprimé, les coords. sont réelles
# 29 mars 2002 (FP) 1.4 aligned_move -> aligned::aligned_move
# 2012-02-16 (FP) 1.5 .esd .bsd
...
proc aligned::hereIsSymdes {machine} {
    set xy [::masque::symDesToPos $::TC(symDes)]
    set ::${machine}(xTheo) [lindex $xy 0]
    set ::${machine}(yTheo) [lindex $xy 1]
}
...
proc aligned::tablexy_ui {machine specialFrameCmd} {
    ...
    bind $b.m.mt <KeyPress-Return> [namespace code [list textpos $machine]]
    ...
    aide::ui_minihelp $bm $b.ep {déclare la position affichée comme position réelle}

    button $b.bsd \
	-command [namespace code "hereIsSymdes $machine"] \
	-text {symDes -> (x,y)}
    aide::ui_minihelp $bm $b.bsd {introduit la position théorique (x,y) à partir du symDes}
	
    frame $b.fsd
    label $b.fsd.lsd -text symDes

    entry $b.fsd.esd \
            -textvariable TC(symDes) \
	    -width 16
    aide::ui_minihelp $bm $b.fsd.esd {nom symbolique du dispositif}
    ...
    button $b.ici -text ici -command [namespace code [list textpos $machine]]

    grid $b.fsd.lsd $b.fsd.esd -sticky news
    
    grid $b.sf        -       -      -       -     -sticky news
    grid $b.fsd       -     $b.lx  $b.ex  $b.m    -sticky news
    grid $b.bsd       -     $b.ly  $b.ey    ^     -sticky news   
    grid $b.ep        -     $b.ldx $b.edx     ^     -sticky news
    grid $b.sc      $b.lc   $b.ldy $b.edy     ^     -sticky news
...
proc aligned::aligned_move {machine v} {
    upvar #0 $machine aligned
