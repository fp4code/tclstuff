...
set polar TM
set DZwarn 1e-4
set eps1 [supercomplex create xy 13.5597711957 0.505943098039]
set eps2 [supercomplex create xy -31.1101462781 0.411896804734]
set lambda 0.80
set d1 0.1
set d2 0.1
set kx0N 0.0
...
set DZwarn 1e-4
set eps1 1
set eps2 [supercomplex create xy -30 0.5]
set lambda 1.0
set d1 2
set d2 0.1
set kx0N 0.0
...
proc plotAll {} {
	...
	foreach r_ym $racines_ym  r_xp $racines_xm  r_yp $racines_yp r_xm $racines_xm {
            incr ir
	    puts stderr [list $ir $r_ym $r_xp $r_yp $r_xm]
	}
    ...
    if {[info exists al(ymm)]} {foreach YN $al(ymm) {
    ...
    if {[info exists al(ymp)]} {foreach YN $al(ymp) {
    ...
    if {[info exists al(xmm)]} {foreach YN $al(xmm) {
	set Z [supercomplex create xy $XNm $YN]
    ...
    if {[info exists al(xmp)]} {foreach YN $al(xmp) {
	set Z [supercomplex create xy $XNp $YN]
	eval .f.c create line [aCurveRe $polar $eps1 $eps2 $d1N $d2N $kx0N $XNm $XNp $Z 1 $dl  $dtheta $DZM1 $ZOOM] -fill black -width 0 -tags xm 
	update
	if {$STOP} return
    ...
    if {[info exists al(xpm)]} {foreach YN $al(xpm) {
	set Z [supercomplex create xy $XNm $YN]
    ...
    if {[info exists al(xpp)]} {foreach YN $al(xpp) {
	set Z [supercomplex create xy $XNp $YN]
    ...
    if {[info exists al(ypm)]} {foreach YN $al(ypm) {
	set Z [supercomplex create xy $XNm $YN]
    ...
    if {[info exists al(ypp)]} {foreach YN $al(ypp) {
	set Z [supercomplex create xy $XNp $YN]
	eval .f.c create line [aCurveIm $polar $eps1 $eps2 $d1N $d2N $kx0N $XNm $XNp $Z -1 $dl $dtheta $DZM1 $ZOOM] -fill orange -width 0 -tag yp
	update
	if {$STOP} return
